...
export const CustomInput = ({ name, type, placeholder, register, req }: CustomInputProps) => {
  return <input id={name} type={type} placeholder={placeholder} className='form__input' {...register(name, req)} />;
};
...
export const RadioInput = ({ name, value, register, req }: RadioInputProps) => {
  return (
    <input className='form__radioinput' type='radio' id={`${name}-${value}`} value={value} {...register(name, req)} />
  );
};
